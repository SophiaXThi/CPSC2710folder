...
public class Part1Controller {
  ...
  public void initialize() {
    ...
    selectMeLabel.textProperty().bindBidirectional(selectMeCheckBox.textProperty());
    numberOfCharactersLabel.textProperty().bind(tweetTextField.textProperty().length().asString());
    validityLabel.textProperty().bind(Bindings.when(tweetTextField.textProperty().length().lessThan(10)).then("Valid").otherwise("Invalid"));
  }
}